...
const razorpay = async (url: string | URL) => {
  ...
  if (u.endsWith('/payment_links')) {
    return new Response(JSON.stringify({ id: 'plink_R', short_url: 'https://rzp.io/i/r', status: 'created', amount: 99800 }), { headers: { 'content-type': 'application/json' } });
  }
...
test('a single failed attempt settles the order as failed', async () => {
  const orderId = await placeOrder([{ id: 'pay_FAIL', status: 'failed', error_code: 'BAD_REQUEST_ERROR', error_description: 'card declined' }]);
  const report = await sweep();
  assert.ok(report.failed >= 1);
  const o = getOrder(orderId)!;
  assert.equal(o.status, 'failed');
  assert.match(String(o.failure_reason), /card declined/);
});
